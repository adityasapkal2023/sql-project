...
FROM Employees WHERE Salary > 60000 GROUP BY DepartmentID;

-- Situation -8: To monitor departmental growth, count the number of employees who joined in
-- 2024 in each department.
SELECT DepartmentID, COUNT(*)
FROM Employees WHERE YEAR(DateOfJoining)=2024 GROUP BY DepartmentID;
...
SELECT DepartmentID, COUNT(*)
FROM Employees WHERE Salary BETWEEN 50000 AND 70000 GROUP BY DepartmentID;

-- Situation -10: For diversity analysis, count the number of employees whose names start with
-- each letter of the alphabet.
SELECT LEFT(FirstName,1) AS Letter, COUNT(*) FROM Employees GROUP BY Letter;
...
-- 4] Situational Questions on CONCAT and Concatenating 
...
SELECT 
CONCAT(Street, ', ', City, ' - ', PostalCode) AS FullAddress
FROM Employees;
...
-- with their position.
SELECT 
CONCAT(DepartmentID, ' - ', Position) AS NotificationInfo
FROM Employees;

-- Situation -9: To display the employee's work status, concatenate their job title and current
-- project.
SELECT 
CONCAT(JobTitle, ' working on ', ProjectName) AS WorkStatus
FROM Employees;
...
SELECT 
CONCAT(YEAR(CURDATE()), '-', FirstName) AS ReportCode
FROM Employees;
...
-- employee within the department.x
...
UPDATE Employees SET LastName = 'Senior Developer'
...
-- communications.
UPDATE Employees SET Email = '9876543210'
...
-- Verify
SELECT * FROM Employees WHERE ProjectName = 'Project Beta';
...
-- Optional population using JOIN
...
SELECT FirstName, Status FROM Employees WHERE FirstName='Test';
